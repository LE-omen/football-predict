...
export async function settleMatch(matchId: string) {
  ...
  for (const market of markets) {
    ...
    if (!preds || preds.length === 0) { await admin.from('markets').update({ market_result: 'void' }).eq('id', market.id); continue; }

    for (const pred of preds) {
      const hit = isMarketHit(market.market_type, { ftHome: match.ft_home_goals, ftAway: match.ft_away_goals, htHome: match.ht_home_goals ?? 0, htAway: match.ht_away_goals ?? 0, selected: pred.selected_option });
      ...
      if (hit && payout > 0) await adjustPoints(pred.user_id, payout, 'hit payout', pred.id);
    }
    await admin.from('markets').update({ market_result: 'won' }).eq('id', market.id);
  }
...
function isMarketHit(marketType: string, args: { ftHome: number; ftAway: number; htHome: number; htAway: number; selected: string }) {
  switch (marketType) {
